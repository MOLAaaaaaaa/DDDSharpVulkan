 ...
 * Shapefile shapefile = new Shapefile();            
   if (shapefile.LoadShapefile("arabcntry.shp"))
   {          
           ...
           Console.WriteLine("Shape {0:n0}, Type {1}", shape.RecordNumber, shape.Type);
            ...
            if (metadataNames != null)
            {
                ...
                foreach (string metadataName in metadataNames)              
                 Console.WriteLine("{0}={1} ({2})", metadataName, 
                 shape.GetMetadata(metadataName), 
...
namespace DataCollection.Shapefile
{
    /// <summary>
    ...
    public class Shapefile : IDisposable, IEnumerable<Shape>
    {
        ...
        //public const string ConnectionStringTemplateJet = @"Provider=Microsoft.Jet.OLEDB.4.0;Data Source={0};Extended Properties=dBase IV";        
        //public const string ConnectionStringTemplateAce = @"Provider=Microsoft.ACE.OLEDB.12.0;Data Source={0};Extended Properties=dBase IV";
        public bool LoadShapefile(string path, string connectionStringTemplate = @"Provider=Microsoft.Jet.OLEDB.4.0;Data Source={0};Extended Properties=dBase IV")
        {
            ConnectionStringTemplate = connectionStringTemplate;
            if (connectionStringTemplate == null || connectionStringTemplate.Length < 5 )
            {
                errMessage = "DB connection string is missing.";
                return false;
            }
            if ( !File.Exists( path ) )
            {
                errMessage = "Shapefile main file not found." + path;
                return false;                
            }
            return Open(path);            
        }
        ...
        public bool Open(string path)
        {
            ...
            {
                errMessage = "Shapefile main file not found." + _shapefileMainPath;
                return false;
            }
            if (!File.Exists(_shapefileIndexPath))
            {
                errMessage = "Shapefile index file not found." + _shapefileIndexPath;
                return false;                
            }
            if (!File.Exists(_shapefileDbasePath))
            {
                errMessage = "Shapefile dBase file not found." + _shapefileDbasePath;
                return false;                
            }
            ...
            {
                errMessage = "Shapefile main file does not contain a valid header.";
                return false;                
            }

            if (_indexStream.Length < Header.HeaderLength)
            {
                errMessage = "Shapefile index file does not contain a valid header.";
                return false;                
            }
        ...
        private bool OpenDb()
        {
            ...
            string connectionString = string.Format(ConnectionStringTemplate, Path.GetDirectoryName(safeDbasePath) );
            _selectString = string.Format(DbSelectStringTemplate,Path.GetFileNameWithoutExtension(safeDbasePath) );

            _dbConnection = new OleDbConnection(connectionString);
            _dbConnection.Open();

            errMessage = "open db successfully.";
