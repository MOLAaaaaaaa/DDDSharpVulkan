...
namespace DataCollection
{
    ...
    {
        Anyway = 0,//Set value to axis anyway
        Lower = 1,//Set value to axis and keep lower one
        Upper = 2,//Set value to axis and keep greater one
    };
...
#pragma warning restore CS0168 // 声明了变量“ex”，但从未使用过
    ...
    static public class ConvertColor
    {
        ...
        static public vec4 Convert(ColorRGBA color)
        {
            float r = color.R / 255f;
            float g = color.G / 255f;
            float b = color.B / 255f;
        ...
        static public Color Convert(vec4 color)
        {
...
#pragma warning restore CS0168 // 声明了变量“e”，但从未使用过
    ...
    public struct CubeModel64
    {
        ...
        public CubeModel64 toCentricCube()
        {
            ...
            double y0 = (Y2 + Y1)/2.0;
            double z0 = (Z2 + Z1)/2.0;
            cube.X1 = x0 - (X2 - X1) / 2.0;
            cube.X2 = x0 + (X2 - X1) / 2.0;
            cube.Y1 = y0 - (Y2 - Y1) / 2.0;
            cube.Y2 = y0 + (Y2 - Y1) / 2.0;
            cube.Z1 = z0 - (Z2 - Z1) / 2.0;
            cube.Z2 = z0 + (Z2 - Z1) / 2.0;
    ...
    public struct Vector32
    {
        ...
        /// </summary>
        /// <param name="n"></param>
        /// <param name="floatNum">小数点位数, < 0 不限</param>
        /// <returns></returns>
        public string toString(int n = 4,int floatNum = -1)
        {
            string ss = "";
            double x1 = x;
            double y1 = y;
            ...
            if( floatNum >= 0 )
            {
                x1 = Math.Round(x, floatNum);
                y1 = Math.Round(y, floatNum);
                z1 = Math.Round(z, floatNum);
                v1 = Math.Round(v, floatNum);
            }
            ss += x1;
            if (n > 1) ss += ",";
            if (n > 1) ss += y1;
        ...
        static public bool TryParse(string text,out Vector32 p, int dimension = 4)
        {
            ...
            p = new Vector32(x1, y1, z1, v1);            
            string[] ss = text.Split(splites, StringSplitOptions.RemoveEmptyEntries);
            if ( ss.Length < dimension ) return false;            
            if (ss.Length > 0) if (!float.TryParse(ss[0], out p.X)) return false;
            if (ss.Length > 1) if (!float.TryParse(ss[1], out p.Y)) return false;
            if (ss.Length > 2) if (!float.TryParse(ss[2], out p.Z)) return false;
            if (ss.Length > 3) if (!float.TryParse(ss[3], out p.V)) return false;
            ss = null;
            return true;
        }
        ...
        /// </summary>
        //search the border of value val in a sorted list
        //n1 lower n2 upper
        static public bool SearchBoder(float val, List<Vector32>points, out int n1, out int n2,double minv,double maxv)
        {           
            n1 = n2 = -1;
            if (val < minv)//超过下界
            {
            ...
            {
                n1 = n2 = points.Count - 1;
                return true;
            }
            else if (val > maxv) //超过上界
            {
                n1 = points.Count - 1;
                n2 = -1;
                return false;
            ...
            BinarySearch(val, points,out n1, out n2);
        ...
        /// <param name="low">返回低值序号</param>
        /// <param name="high">返回高值序号</param>
        /// <returns> <0 未找到 </returns>
        static public int BinarySearch(float val, List<Vector32> points, out int low, out int high)
        {
            low = 1;
            high = points.Count - 1;
            int mid = (low + high) / 2;
        ...
        static public Vector32 Parse(string text, int dimension = 4)
        {
            float x1 = 0, y1 = 0, z1 = 0, v1 = 0;
            char[] splites = new char[] { ',', ' ', '\t' };
            Vector32 p = new Vector32(x1, y1, z1, v1);
            string[] ss = text.Split(splites, StringSplitOptions.RemoveEmptyEntries);
            if (ss.Length < dimension) return p;
            if (ss.Length > 0) if (!float.TryParse(ss[0], out p.X)) return p;
            if (ss.Length > 1) if (!float.TryParse(ss[1], out p.Y)) return p;
            if (ss.Length > 2) if (!float.TryParse(ss[2], out p.Z)) return p;
            if (ss.Length > 3) if (!float.TryParse(ss[3], out p.V)) return p;
            ss = null;
            return p;
        }
        ...
        /// <returns></returns>
        static public bool IsEqual(Vector32 p1, Vector32 p2,double err = 1.0E-10 )
        {
            double xx = Math.Abs(p1.x - p2.x);
            double yy = Math.Abs(p1.y - p2.y);
            double zz = Math.Abs(p1.z - p2.z);
            if (xx + yy + zz <= err) return true;
            else return false;
        }
        public bool IsEqual(Vector32 p1, double err = 1.0E-10)
        {
            double xx = Math.Abs(p1.x - X);
            double yy = Math.Abs(p1.y - Y);
            double zz = Math.Abs(p1.z - Z);
            if (xx + yy + zz <= err) return true;
            else return false;
        }
        public override bool Equals(object obj)
        ...
        /// <returns></returns>
        static public List<Vector32> Resample(List<Vector32> points, int ptNum = 101)
        {
            //采样间隔
            double step = GetLength(points) / (ptNum - 1);

            int n = points.Count;
            List<Vector32> line = new List<Vector32>();
        ...
        /// <returns></returns>
        static public int RemoveDuplicated(ref List<Vector32> points, double zerobase = 1.0E-10)
        {
        ...
        static public int RemoveLineDuplicated(ref List<Vector32> points, double ZeroBase = 0.001)
        {
            ...
            for (int i = 1; i < n - 1; i++)
            {
                p1 = points[id1];
                p2 = points[i];
                if (p1.Distance(p2) <= zero) mark[i] = true;
                else id1 = i;
            }
        ...
        /// <returns></returns>
        /////-------------------
        static public int RemoveLineRedundant(ref List<Vector32> points, double tanErr = 1.0E-3)
        {
            ...
            while (lists.Count > 0)
            {
                ...
                line.p1 = p1.toVector64();
                line.p2 = p2.toVector64();

                if (line.IsOnLineExt(p.toVector64(), 1.0E-6))
                {
                    //删除前一点
        ...
        public static double Distance(Vector32 p1, Vector32 p2,int pow = 2)
        {
            if (pow == 1) //曼哈顿距离
        ...
        public static double GetLength(List<Vector32> points, int index = -1)
        {
            int n = points.Count;
            if (n < 2) return 0;
        ...
        /// 将p点捕捉到最近的点上
        /// </summary>
        /// <param name="p">待捕捉点</param>
        /// <param name="points">点数组</param>        
        /// <param name="snaped">捕捉点</param>        
        /// <returns>捕捉距离</returns>
        static public double SnapOnPointsGrid( Vector32 p, List<Vector32> points, out Vector32 snaped )
        {
        ...
        /// </summary>
        /// <param name="p">待捕捉点</param>
        /// <param name="points">线点数组</param>
        /// <param name="IsClosed">是否封闭线</param>
        /// <param name="snaped">捕捉点</param>
        /// <param name="distance">捕捉距离</param>
        /// <returns>捕捉是否成功</returns>
        static public bool SnapOnEdgeLines(Vector32 p,List<Vector32> points, bool IsClosed,out Vector32 snaped, out double distance)
        {
            Vector32 p1, p2, p0;
            ...
            for (int i = 0; i < points.Count; i++)
            {
                ...
                dist = line.Distance(p.toVector64());
                if (dist < mindist)
                {
                    p0 = line.GetPointProjection(p.toVector64());//点到直线投影点
                    if (line.IsOnLine(p0.toVector64()))
                    {
                        snaped = p0;
                        mindist = dist;
                        found = true;
    ...
    public struct Vector64
    {
        ...
        //search the border of value val in a sorted list
        //n1 lower n2 upper
        static public bool SearchBoder(double val, List<Vector64> points, out int n1, out int n2, double minv, double maxv)
        {
            n1 = n2 = -1;
            if (val < minv)//超过下界
            {
        ...
        /// <param name="high">返回高值序号</param>
        /// <returns> <0 未找到 </returns>
        static public int BinarySearch(double val, List<Vector64> points, out int low, out int high)
        {
            low = 1;
            high = points.Count - 1;
            int mid = (low + high) / 2;
        ...
        static public bool TryParse(string text, out Vector64 p, int dimension = 4)
        {
            double x1 = 0, y1 = 0, z1 = 0, v1 = 0;
            char[] splites = new char[] { ',', ' ', '\t' };
            p = new Vector64(x1, y1, z1, v1);
            string[] ss = text.Split(splites, StringSplitOptions.RemoveEmptyEntries);
            if (ss.Length < dimension) return false;
            if (ss.Length > 0) if (!double.TryParse(ss[0], out p.X)) return false;
            if (ss.Length > 1) if (!double.TryParse(ss[1], out p.Y)) return false;
            if (ss.Length > 2) if (!double.TryParse(ss[2], out p.Z)) return false;
            if (ss.Length > 3) if (!double.TryParse(ss[3], out p.V)) return false;
            ss = null;
            return true;
        }
        static public Vector64 Parse(string text, int dimension = 4)
        {
            double x1 = 0, y1 = 0, z1 = 0, v1 = 0;
            char[] splites = new char[] { ',', ' ', '\t' };
            Vector64 p = new Vector64(x1, y1, z1, v1);
            string[] ss = text.Split(splites, StringSplitOptions.RemoveEmptyEntries);
            //if (ss.Length < dimension) return p;
            if (ss.Length > 0) if (!double.TryParse(ss[0], out p.X)) return p;
            if (ss.Length > 1) if (!double.TryParse(ss[1], out p.Y)) return p;
            if (ss.Length > 2) if (!double.TryParse(ss[2], out p.Z)) return p;
            if (ss.Length > 3) if (!double.TryParse(ss[3], out p.V)) return p;
            ss = null;
            return p;
        }
        public override int GetHashCode()
        ...
        static public List<Vector64> Resample(List<Vector64> points,int ptNum = 101)
        {            
            //采样间隔
            double step = GetLength(points) / (ptNum - 1);

            int n = points.Count;
            List<Vector64> line = new List<Vector64>();
        ...
        /// </summary>
        /// <param name="points"></param>
        /// <param name="tanErr">斜率变化误差</param>
        /// <returns></returns>
        /////-------------------
        static public int RemoveLineRedundant(ref List<Vector64> points,double tanErr = 1.0E-3)
        {
            if (points.Count < 3) return 0;
        ...
        /// <param name="err"></param>
        /// <returns></returns>
        static public bool IsEqual(Vector64 p1, Vector64 p2, double err = 1.0E-10)
        {
            double xx = Math.Abs(p1.x - p2.x);
            double yy = Math.Abs(p1.y - p2.y);
            double zz = Math.Abs(p1.z - p2.z);
        ...
        public bool IsEqual(Vector64 p1, double err = 1.0E-10)
        {
            double xx = Math.Abs(p1.x - X);
            double yy = Math.Abs(p1.y - Y);
            double zz = Math.Abs(p1.z - Z);
        ...
        /*! @param PIP Point-in-Plane */
        public static bool TestLineThruTriangle(Vector64 P1, Vector64 P2, Vector64 P3, Vector64 R1, Vector64 R2, out Vector64 PIP)
        {
            PIP = new Vector64(0, 0, 0);
            // Find Triangle Normal
            Vector64 Normal = Cross(P2 - P1, P3 - P1);
            Normal.Normalize(); // not really needed?  Vector3f does this with cross.
            ...
            // Find point on the line that intersects with the plane
            Vector64 IntersectPos = R1 + (R2 - R1) * (-Dist1 / (Dist2 - Dist1));

            // Find if the interesection point lies inside the triangle by testing it against all edges            
            Vector64 vTest = Normal.Cross(P2 - P1);
            if (vTest.Dot(IntersectPos - P1) < 0.0f)
            {
                //SFLog(@"no intersect P2-P1"); 
                return false;
        ...
        public static double Distance(Vector64 p1, Vector64 p2, int pow = 2)
        {
            if(pow ==1) //曼哈顿距离
                return Math.Abs(p1.X - p2.X) + Math.Abs(p1.Y - p2.Y) + Math.Abs(p1.Z - p2.Z);
            else if (pow == 4) //距离平方
        ...
        /// <param name="snaped">捕捉点</param>        
        /// <returns>捕捉距离</returns>
        static public double SnapOnPointsGrid(Vector64 p, List<Vector64> points, out Vector64 snaped)
        {
            double mindist = 1.0E20;
            double dist;
            ...
            foreach (Vector64 p1 in points)
            {
                dist = p1.Distance(p,4);
                if (dist < mindist)
                {
                    snaped = p1;
                    mindist = dist;
        ...
        /// <param name="distance">捕捉距离</param>
        /// <returns>捕捉是否成功</returns>
        static public bool SnapOnEdgeLines(Vector64 p, List<Vector64> points, bool IsClosed, out Vector64 snaped, out double distance)
        {
            Vector64 p1, p2, p0;
            ...
            for (int i = 0; i < points.Count; i++)
            {
                ...
                dist = line.Distance(p,4);
                if (dist < mindist)
                {
                    p0 = line.GetPointProjection(p);//点到直线投影点
                    if ( line.IsOnLine(p0) )
                    {
                        snaped = p0;
                        mindist = dist;
    ...
    {
        public Vector64 p1, p2, p3;
        public ColorRGBA[] pColor;
        public Vector64 pNormal;
        ...
        public Vector64 Center
        {
            get 
            {
                return new Vector64((p1.x + p2.x + p3.x) / 3,
                                    (p1.y + p2.y + p3.y) / 3, 
                                    (p1.z + p2.z + p3.z) / 3);
            }
        }
        public CTriangle3f(Vector64 _p1, Vector64 _p2, Vector64 _p3)
        {
            p1 = _p1;
            p2 = _p2;
            p3 = _p3;
            pColor = new ColorRGBA[3];
            pNormal = new Vector64(0, 0, 0);
            bIsConverted = false;
        }
        ...
        public int GetRelationOfPoint(Vector64 p)
        {
            Vector64 p32 = p2 - p3;
            Vector64 p31 = p1 - p3;
            Vector64 p01 = Vector64.Cross(p32, p31);
            Vector64 p02 = p - p3;
            double dd = Math.Round(Vector64.Dot(p01, p02),10);
            if (dd > 0) return 1;
        ...
        /// <param name="P"></param>
        /// <returns></returns>
        public bool IsPointInTriangle(Vector64 P)
        {
            Vector64 v0 = p3 - p1;
            Vector64 v1 = p2 - p1;
            Vector64 v2 = P - p1;
        ...
        public IntersectionType GetPointTriangleType(Vector64 p)
        {
            // on the corner points
            if (p == p1 || p == p2 || p == p3)
                return IntersectionType.corner;
            ...
            if (u + v <= 1)
            {
                if (Vector64.IsPointOnLine(p1, p2, p)) return IntersectionType.line;
                if (Vector64.IsPointOnLine(p2, p3, p)) return IntersectionType.line;
                if (Vector64.IsPointOnLine(p1, p3, p)) return IntersectionType.line;
        ...
        /// <param name="P"></param>
        /// <returns></returns>
        public IntersectionType GetIntersectionType(Vector64 P)
        {
            // on the corner points
            if (P == p1 || P == p2 || P == p3)
                return IntersectionType.corner;
        ...
        public IntersectionType CheckLineCrossTriangle(Vector64 v1, Vector64 v2, int direct)
        {
            //triangle out of the range of line h1---h2 
            double minx, maxx, miny, maxy, minz, maxz;
            GetRange(out minx, out maxx, out miny, out maxy, out minz, out maxz);
            Vector64 sect = new Vector64(0, 0, 0);
            switch (direct)
            {
                case 0://x direction
                    ...
                    {
                        if (Vector64.IsPointOnLine(v1, p1, p3))
                            return IntersectionType.line;
                    ...
                    {
                        if (Vector64.IsPointOnLine(v1, p1, p2))
                            return IntersectionType.line;
                        else return IntersectionType.none;
                    }
        ...
        public bool GetIntersectionOnTriangle(Vector64 v1, Vector64 v2, out Vector64 intersect)
        {
            double t, u, v;
            Vector64 orig = v1;
            Vector64 dir = v2 - v1;
            ...
            intersect = new Vector64(orig.x, orig.y, orig.z);
        ...
        */
        ...
        static public bool ValidTriangle(Vector64 _p1, Vector64 _p2, Vector64 _p3)
        {
            if (Vector64.GetPointLineRelation(_p1, _p2, _p3) == 0)
                return false;
            else return true;
        }
        ...
        int GetLineIntersection(Vector64 start, Vector64 end, out Vector64 CrossPoint)
        {
            SpaceVector LineV = new SpaceVector();
            Vector64 line = end - start;
            LineV.m = line.X;
            LineV.n = line.Y;
            LineV.p = line.Z;
            //三角形所在平面的法向量
