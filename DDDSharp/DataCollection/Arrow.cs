///////////////////////////////////////////////////////
/// Draw Straight or Curved Arrow , written by Michal Brylka 2007
/// modified by jian 2020-3-12
/// https://www.codeproject.com/Articles/17417/Draw-Straight-and-Curved-Arrows
///
...
namespace DataCollection
{
    public class ArrowRenderer
    {
        ...
        public void DrawArrow(Graphics g, Pen pen, Brush brush, PointF p1, PointF p2)
        {
            ...
            PointF pBase = new PointF((float)(aptArrowHead[0].X + -ta * vecLine.X), (float)(aptArrowHead[0].Y + -ta * vecLine.Y)); //base of the arrow

            // build the points on the sides of the arrow
            aptArrowHead[1] = new PointF((float)(pBase.X + th * vecLeft.X), (float)(pBase.Y + th * vecLeft.Y) );
            aptArrowHead[2] = new PointF((float)(pBase.X + -th * vecLeft.X), (float)(pBase.Y + -th * vecLeft.Y));
        ...
        public void DrawArrows(Graphics g, Pen pen, Brush brush, PointF[] apf, float startOffset, float endOffset)
        {
            ...
            {
                toP = newApf[i + 1];
                Vector64 vecLine = new Vector64(toP.X - fromP.X, toP.Y - fromP.Y, 0);                
                double scale = 1 - (vecLine.Length - startOffset) / vecLine.Length;
                //Vector vecStart = vecLine - (vecLine.Length - startOffset);
                Vector64 vecStart = scale * vecLine;
                PointF nFromP = new PointF((float)(fromP.X + vecStart.X), (float)(fromP.Y + vecStart.Y));
                //Vector vecEnd = vecLine - endOffset;
                Vector64 vecEnd = vecLine * (1.0 - endOffset / vecLine.Length);
                PointF nToP = new PointF((float)(fromP.X + vecEnd.X), (float)(fromP.Y + vecEnd.Y));
        ...
        public void DrawArrows(Graphics g, Pen pen, Brush brush, PointF[] apf, float[] startOffsets, float[] endOffsets)
        {
            ...
            for (int i = 0; i < newApf.Length - 1; i++)
            {
                ...
                Vector64 vecStart = (1.0 - (vecLine.Length - startOffsets[i]) / vecLine.Length) * vecLine;
                PointF nFromP = new PointF((float)(fromP.X + vecStart.X), (float)(fromP.Y + vecStart.Y));
                //Vector vecEnd = vecLine - endOffsets[i];
                Vector64 vecEnd = (1.0 - endOffsets[i] / vecLine.Length) * vecLine;
                PointF nToP = new PointF((float)(fromP.X + vecEnd.X),(float)( fromP.Y + vecEnd.Y) );
                DrawArrow(g, pen, brush, nFromP, nToP);
                fromP = toP;
            }
