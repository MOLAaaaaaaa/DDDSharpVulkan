...
{
   vec4 shininess;   // shiniess.w = 128,shiniess.x = ns,shiniess.y = speed,shiniess.z = time
   vec4 emission;    // Ecm   
   vec4 ambient;     // Acm   
   vec4 diffuse;     // Dcm   
   vec4 specular;    // Scm
...
layout(location = 2) out vec4 texLight;
...
void main() 
{
	...
	vec4 inColor1 = inColor;
	if( inTexCoord.x >= 0.0 && inTexCoord.y >= 0.0 )
	{
	     inColor1 = vec4(1,1,1,1);
	}
	...
	for(int i=0;i<8;i++)
	{
		if( ubo.lights[i].type.w > 0)
		{
			...
			cal_ambient = inColor1 * (ubo.lights[i].ambient * material_ambient);		
			
			cal_diffuse =  inColor1 * light_diffuse * max( 0.0, dot(N, L) );
			cal_diffuse = cal_diffuse * material_diffuse;
			
			cal_specular =  light_specular *  pow( max(dot(N, H), 0.0), shininess ) * ns;			
			cal_specular = cal_specular * inColor1 * material_specular;
	...
	if( inTexCoord.x >= 0.0 && inTexCoord.y >= 0.0 )
	{
	     	texLight = vec4( clamp( color, 0, 1 ) );
		fragColor = inColor;
		float wavetime = ubo.material.shininess.z;
		if( wavetime > 0 ) //波动纹理
		{
		  fragTexCoord.x = fragTexCoord.x + sin(wavetime);
		  fragTexCoord.y = fragTexCoord.y + cos(wavetime);
		  fragTexCoord = clamp(fragTexCoord,0,1);
		}
	}
	else
	{
